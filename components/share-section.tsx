...
export function ShareSection({ result }: ShareSectionProps) {
  ...
  const whatsappMessage =
    result.salePrice > 0
      ? `¡Oferta en Bodega Don Pepe! ${result.productName} a solo S/. ${result.salePrice.toFixed(2)}. ¡Ven antes que se acabe!`
    ...
    <Card className="shadow-md border-border/50 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-150">
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground text-center">
          Función de compartir deshabilitada en este prototipo
        </p>
      </CardContent>
    </Card>
  )
